 ...
 //   console.log("processServerCommand", protocolKey, message)
    ...
    //        console.log("Player Connected; ", stamp, msg);
        ...
        //    console.log("LOAD_FILE_DATA:", message);
            ...
            break;
        case ENUMS.ServerCommands.PROCESS_CHAT_MESSAGE:
            console.log("PROCESS_CHAT_MESSAGE:", message);

            if (stamp === client.getStamp()) {
                message.info.name = "You"
                let actor = getPlayerActor();
                if (actor) {
                    actor.actorText.say(message.text);
                }

            } else {
                let rCli = remoteClients[stamp] || null;
                if (rCli !== null) {
                    console.log("message from remote:", rCli);
                    message.info.name = rCli.getRemotePlayerName();
                    rCli.applyRemoteChatMessage(message.text);
                } else {
                    console.log("message from non-remote:", message);
                    message.info.name = "system";
                }
            }

            evt.dispatch(ENUMS.Event.CHAT_MESSAGE, {text:message.text, info:message.info})
