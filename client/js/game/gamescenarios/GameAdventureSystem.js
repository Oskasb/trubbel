...
import {getPlayerStatus, setStatusValues} from "../../application/utils/StatusUtils.js";
    ...
    constructor() {
        ...
        let updateActiveAdventure = function(advId) {
            ...
            function update() {
                ...
                let currentWl = GameAPI.getPlayer().getStatus(ENUMS.PlayerStatus.PLAYER_WORLD_LEVEL)
                if (currentWl === getPlayerStatus(ENUMS.PlayerStatus.PLAYER_ID)) {
                    currentWl = "19";
                }
            if (worldLevel !== currentWl) {
                worldLevel = currentWl;
            ...
            let adventureCompleted = function(wAdv) {
                ...
                if (dataList.indexOf(wAdv.id) === -1) {
                    ...
                    if (wAdvs.indexOf(wAdv) !== -1) {
                        ...
                        console.log("completed aFdventure should be present as active until completion")
                    }
                    ...
                    let onCompleted = wAdv.config['on_completed'];

                    if (onCompleted) {
                        let setStatus = onCompleted['set_status'];

                        if (setStatus) {
                            setStatusValues(setStatus)
                        }
