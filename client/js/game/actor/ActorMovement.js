...
import {
    ...
    getBodyByPointer, processRayHitKickInfluence,
    rayTest,
    testProbeFitsAtPos
...
class ActorMovement {
   ...
   //         GuiAPI.screenText("SHORT")
    ...
    probeMovementPhysics(actor, inputAmount, probeShape) {
        ...
        probeResult.translation.multiplyScalar(inputAmount * moveSpeed * 0.5)
    ...
    //    probeResult.translation.applyQuaternion(quat);
        ...
        let hit = rayTest(probeResult.from, probeResult.to, probeResult.destination, tempNormal, false, true)
        if (!hit) {
        ...
        //    processRayHitKickInfluence(hit, probeResult.translation, getSetting(ENUMS.Settings.ACTOR_INFLUENCE_POWER), hit.position)
        ...
        let groundHeight = ThreeAPI.terrainAt(probeResult.destination, tempNormal) -0.25;
        ...
        hit = rayTest(probeResult.destination, probeResult.to, probeResult.destination, tempNormal, false, true)
    ...
    //    probeResult.to.y += 1.7;
        ...
        //    processRayHitKickInfluence(hit, probeResult.translation, getSetting(ENUMS.Settings.ACTOR_INFLUENCE_POWER), hit.position)
            ...
            if (normalY > 0.3 && probeResult.destination.y > -1) {
                probeResult.blocked = false;
            } else {
                probeResult.blocked = true;
            }
...
*/
    ...
    //    this.visualArc.to.y = targetElevation;
        ...
        let probeRes = this.probeMovementPhysics(actor, MATH.sign(forward)*0.5+forward*0.5,  physicsProbes[1]);
    ...
    //        actor.actorText.say("Leap")
        //    walkGrid.dynamicWalker.attachFrameLeapTransitionFx(actor)
            let onArrive = function(pos, spatTransition) {
                activatePhysicalShockwave(actor.getPos(), 1.5 + spatTransition.targetTime * 0.95, 0.15 + spatTransition.targetTime*0.1, 4 + spatTransition.targetTime, 'GLITTER_FX')
        ...
        //        actor.actorText.say("Distance "+MATH.numberToDigits(distance, 1, 1)+"m")
            ...
            let overTime = 0.2+MATH.curveSqrt(distance*4)*0.15
            activatePhysicalShockwave(fromVec, 1 + overTime * 0.7, 0.1  + overTime*0.15, 4 + overTime, 'GLITTER_FX')
            let toVec = actor.getDestination();
            transition.initSpatialTransition(fromVec, toVec, overTime, onArrive, distance*0.6, 'curveEdge', onFrameUpdate)
