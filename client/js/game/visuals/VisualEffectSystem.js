...
function setupOptsVelocityFx(efct, pos) {
...
//    tempObj.position.y += 0.5
    ...
    ThreeAPI.tempVec3.set(0.4, 0.4, 0.4)
    MATH.spreadVector(tempVec3, ThreeAPI.tempVec3)
...
let spawnEffect = function(event) {
    ...
    if (event.effectConfig === 'additive_particles_8x8') {
        let effectCb = function(efct) {
            efct.activateEffectFromConfigId()
            let options = setupOptsVelocityFx(efct, event.pos)
            options.fromSize *= event.fromSize;
            options.toSize *= event.toSize;
            if (event.duration) {
                options.time *= event.duration;
            }
        ...
        EffectAPI.buildEffectClassByConfigId( 'additive_particles_8x8', event.effectName || 'particle_additive_pool',  effectCb)
