...
class WorldModel {
    ...
    //    console.log("New World Model", config, id)
        ...
        let worldModelLodUpdate = function(lodLevel) {
            ...
            let visibility = 3 + getSetting(ENUMS.Settings.VIEW_DISTANCE)
            ...
            } else if (MATH.valueIsBetween(lodLevel, 2, visibility)) {
                    if (modelsLoaded === false) {
                        locationModels(this.configData);
                    }
                    setLocModelsLod(this.locationModels, lodLevel, visibility);
                 lodDeactivate()
...
//            this.obj3d.copy(obj3d);
    ...
    imprintWorldModelToGround(imprintCallback) {
        this.calcBounds();
        let box = this.box;
        this.call.worldModelLodUpdate(0);
