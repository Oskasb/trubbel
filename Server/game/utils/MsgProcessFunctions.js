...
import {
    ...
    getServerStamp, getStatusFromMsg, registerServerItem, statusMapFromMsg
    ...
    switch (request) {
        ...
        case ENUMS.ClientRequests.LOAD_SERVER_ITEM:
            ...
            if (typeof (message.status) === 'string') {
                serverItem = getServerItemByItemId(message.status)
            } else {
                serverItem = new ServerItem(message.status, connectedClient.stamp);
 ...
 */
            ...
            break;
        case ENUMS.ClientRequests.SEND_CHAT_MESSAGE:
            console.log("send chat message", message)
            message.command = ENUMS.ServerCommands.PROCESS_CHAT_MESSAGE;
            message.info.clientId = message.stamp;
