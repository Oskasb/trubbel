...
import {
    ...
    passSequencerTurnToActor, selectActorEncounterTarget,
    startEncounterTurn,
    ...
    constructor(serverEncounter) {
        ...
        function updateEncounterActorStatus(actors) {
            ...
            for (let i = 0; i < actors.length; i++) {
                ...
                if (encActor.getStatus(ENUMS.ActorStatus.ATTITUDE) === 'HOSTILE') {
        ...
        let updateTurnSequencer = function() {
            ...
            let inSync = this.serverEncounter.call.testPlayersAreInSync();

            if (!inSync) {
                console.log("Players Not yet in sync")
                serverEncounter.sendEncounterStatusUpdate(true)
                setTimeout(updateTurnSequencer, 200)
                return;
            } else {
            //    console.log("Players Synched")
            }
            ...
            //    console.log("Update TURN_INIT", actor.id)
                ...
                if (serverEncounter.actorIsPlayer(actor)) {
                    let candidate = selectActorEncounterTarget(serverEncounter, actor)
                    if (candidate) {
                        applyEncounterTargetSelection(serverEncounter, actor, candidate)
                    } else {
