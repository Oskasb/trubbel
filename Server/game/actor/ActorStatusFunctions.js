...
function enterEncounter(encounter, actor) {
    ...
    console.log("Enter Encounter ", actor.getStatus(ENUMS.ActorStatus.ACTOR_ID), actor.getStatus(ENUMS.ActorStatus.MAX_HP))
    ...
    actor.setStatusKey(ENUMS.ActorStatus.ENCOUNTER_UPDATE_INDEX, encounter.getStatus(ENUMS.EncounterStatus.UPDATE_INDEX));
...
function updateActorEncounterEngagements(actor, encounter) {
    ...
    for (let i = 0; i < engagedTargets.length; i++) {
        ...
        if (!target) {
            console.log("No such target", engagedTargets[i]);
            MATH.splice(engagedTargets, engagedTargets[i]);
            i--
        } else {
